...
class Controller
{
	...
	public function xoadanhmuc() {
		if (!empty($_GET['id'])) {
			$danhmuc_id = $_GET['id'];
			if ($this->model->deleteDanhMuc($danhmuc_id)) {
				$ketqua = true; // Xoá thành công
			} else {
				$ketqua = false; // Xoá thất bại
			}
		} else {
			$ketqua = false;
		}
		require_once('./view/xoadanhmuc.php');
	}
	public function xoasanpham() {
		if (!empty($_GET['id'])) {
			$sanpham_id = $_GET['id'];
			if ($this->model->deleteSanpham($sanpham_id)) {
				$ketqua = true; // Xoá thành công
			} else {
				$ketqua = false; // Xoá thất bại
			}
		...
		require_once('./view/xoasanpham.php');
	}
	...
	public function themsanpham(){
		...
		if($_SERVER["REQUEST_METHOD"] == "POST"){ 
			...
			if(!isset($error_danhmuc)&&!isset($error_ten)&&!isset($error_gia)&&!isset($error_mota)&&!isset($error_anhdaidien)&&!isset($error_soluong)){
